...
def complexes_to_csv(complexes, prot_ids=[], pvalue_dict=None, genename_cannotfind_errors=None, genename_nocomplex_errors=None):
    ...
    for comp in complexes:
        complexes_dict['complexmap_id'].append(comp.complexmap_id)
    ...
    if genename_cannotfind_errors != None:
        complexes_dict['complexmap_id'].append("COULD_NOT_FIND_GENENAMES")
    ...
    if genename_nocomplex_errors != None:
        complexes_dict['complexmap_id'].append("NO_COMPLEXES_FOR_GENENAMES")
        complexes_dict['rank'].append(None)
        if pvalue_dict != None:
            complexes_dict['pvalue'].append(None)
        if len(prot_ids) > 0:
...
def displayComplexes():
    ...
    try:
        ...
        comp = db.session.query(cdb.Complex).filter_by(complexmap_id=complex_key).one()
    except NoResultFound:
        comp = None
